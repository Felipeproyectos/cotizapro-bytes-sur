...
export default function OperationalExpenses() {
  ...
  // Extraer todos los gastos operacionales de todas las cotizaciones
  ...
  const filtered = allExpenses.filter((exp) => {
        ...
        const matchMonth =
                !filterMonth ||
                (exp.quoteDate && exp.quoteDate.startsWith(filterMonth));
        return matchSearch && matchMonth;
  });

  const totalGastos = filtered.reduce((sum, exp) => sum + (exp.total || 0), 0);

  // Agrupar por cotización para resumen
            ...
            {/* Header */}
                <div className="bg-white border-b border-gray-100 px-6 py-5">
                        <div className="flex items-center gap-3 mb-1">
                                  ...
                                  <h1 className="text-xl font-bold text-slate-900">Gastos Operacionales</h1>h1>
                        </div>div>
                ...
                <div className="p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                                              <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <p className="text-sm text-slate-500 font-medium">Total Gastos (filtro)</p>p>
                                              </div>div>
                                              <p className="text-2xl font-bold text-red-600">{formatCLP(totalGastos)}</p>p>
                                  </div>div>
                                  <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                                              <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <p className="text-sm text-slate-500 font-medium">Cotizaciones con Gastos</p>p>
                                              </div>div>
                                              <p className="text-2xl font-bold text-slate-900">{quoteSummaries.length}</p>p>
                                  </div>div>
                                  <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                                              <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <p className="text-sm text-slate-500 font-medium">Ítems de Gasto</p>p>
                                              </div>div>
                                              <p className="text-2xl font-bold text-slate-900">{filtered.length}</p>p>
                                  </div>div>
                        </div>div>
                  ...
                  {quoteSummaries.length === 0 ? (
                      <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
                                  ...
                                  <p className="text-slate-500 font-medium">No hay gastos operacionales registrados</p>p>
                                                                          ...
                                                                          <div className="px-5 py-4 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                                                                              <div>
                                                                                                                    <div className="flex items-center gap-2 mb-1">
                                                                                                                                            <span className="text-xs font-mono text-slate-400">#{q.quoteNumber}</span>span>
                                                                                                                                            <span
                                                                                                                    ...
                                                                                                                    <p className="text-sm font-semibold text-slate-900">{q.quoteTitle}</p>p>
                                                                                                                    <p className="text-xs text-slate-500">{q.clientName}</p>p>
                                                                                                </div>div>
                                                                                              <div className="text-right">
                                                                                                                    <p className="text-xs text-slate-400 mb-1">Total gastos en esta cotización</p>p>
                                                                                                                    <p className="text-lg font-bold text-red-600">{formatCLP(q.total)}</p>p>
                                                                                                </div>div>
                                                                          </div>div>
                                                                            ...
                                                                            {q.items.map((item, iIdx) => (
                                                                                <div key={iIdx} className="px-5 py-3 flex items-center justify-between">
                                                                                                        <div className="flex items-center gap-3">
                                                                                                                                  ...
                                                                                                                                  <div>
                                                                                                                                                              <p className="text-sm font-medium text-slate-800">{item.service_name || "Sin nombre"}</p>p>
                                                                                                                                    {item.description && (
                                                                                                                <p className="text-xs text-slate-400">{item.description}</p>p>
                                                                                                                                                              )}
                                                                                                        ...
                                                                                                        <p className="text-sm font-semibold text-slate-900">{formatCLP(item.total || 0)}</p>p>
                                                                                  </div>div>
                                                                              ))}
                                                                          </div>div>
                                                        </div>div>
