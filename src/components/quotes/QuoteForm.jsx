...
export default function QuoteForm({ quote, onSave, onCancel }) {
  ...
  const recalc = (f) => {
    ...
    const iva_amount = f.include_iva ? subtotal * IVA_RATE : 0;
    const total = subtotal + iva_amount;
    setForm({ ...f, subtotal, iva_amount, total });
  };
  ...
  const setField = (field, val) => {
    ...
    if (field === "include_iva") {
      ...
      updated.iva_amount = val ? subtotal * IVA_RATE : 0;
  ...
  const handleSave = async () => {
    ...
    const iva_amount = form.include_iva ? subtotal * IVA_RATE : 0;
    const payload = { ...form, subtotal, iva_amount, total: subtotal + iva_amount };
    if (quote?.id) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Client */}
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <h2 className="text-sm font-semibold text-slate-900 mb-4">Datos del Cliente</h2>
            ...
            <div key={idx} className="border border-gray-100 rounded-xl p-4 bg-gray-50/50">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                <div className="md:col-span-4">
                  ...
                  <select
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-900/10"
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded"
                checked={form.include_iva} onChange={e => setField("include_iva", e.target.checked)} />
              <span className="text-sm text-slate-500">IVA (19%)</span>
            </label>
            <span className="font-semibold text-slate-900 w-32 text-right">{form.include_iva ? formatCLP(iva_amount) : "—"}</span>
          </div>
