...
const ROOT_PAGES = ["Dashboard", "Quotes", "History", "OperationalExpenses", "Services", "Settings"];
              ...
              <aside className="hidden md:flex flex-col w-64 min-h-screen bg-white border-r border-gray-100 fixed left-0 top-0 bottom-0 z-30">
                      ...
                      <div className="flex flex-col items-center gap-2">
                                    ...
                                    <div className="text-center min-w-0">
                                                    <p className="text-sm font-bold text-slate-900 truncate leading-tight">{company.company_name}</p>p>
                                      {company.rut && <p className="text-xs text-slate-400 truncate font-mono">{company.rut}</p>p>}
                                    </div>div>
                      </div>div>
                    ) : (
                      <div className="flex flex-col items-center gap-2">
                                    ...
                                    <div className="text-center">
                                                    <p className="text-sm font-bold text-slate-900">{company?.company_name || "CotizaPro"}</p>p>
                                                    <p className="text-xs text-slate-400">Panel de gestión</p>p>
                                    </div>div>
                      </div>div>
                                )}
                      </div>div>
                      ...
                      <div className="px-4 py-5 border-t border-gray-100">
                                <p className="text-xs text-slate-400 text-center">© 2026 WWW.SOLUCIONESFML.CL</p>p>
                      </div>div>
              </aside>aside>
                ...
                {navItems.map(({ name, page, icon: Icon }) => {
                                  ...
                                  return (
                                                <Link
                                                              ...
                                                              <span className={`text-[10px] font-medium ${isActive ? "text-slate-900" : "text-slate-400"}`}>{name}</span>span>
                                                </Link>Link>
                                              );
                      })}
              </nav>nav>
